...
open class CountryPicker: UITableViewController {
    ...
    fileprivate var unsortedCountries : [Country] {
        ...
        for countryCode in countriesCodes {
            ...
            if countryData.count > 0, let dialCode = countryData[0]["dial_code"] {
                country = Country(name: displayName!, code: countryCode, dialCode: dialCode, flagImg: nil)
            } else {
                country = Country(name: displayName!, code: countryCode, flagImg: nil)
            }
    ...
    fileprivate var sections: [Section] {
        ...
        let countries: [Country] = unsortedCountries.map { country in
            let country = Country(name: country.name, code: country.code, dialCode: country.dialCode, flagImg: nil)
        ...
        for section in sections {
            var s = section
             let str = #selector(getter: Country.name)
            s.countries = collation.sortedArray(from: section.countries, collationStringSelector: str) as! [Country]
        }
    ...
    open var defaultCountryCode = "IN"
    ...
    override open func viewDidLoad() {
        ...
        if self.presentingViewController != nil {
            ...
            let closeButton = UIBarButtonItem(image: UIImage(named: bundle + "close_icon" + ".png",
                                                             in: Bundle(for: CountryPicker.self),
                                                             compatibleWith: nil),
                                              style: .plain,
                                              target: self,
...
extension CountryPicker {
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.showFlags == true {
            let image = UIImage(named: bundle + country.code.uppercased() + ".png", in: Bundle(for: CountryPicker.self), compatibleWith: nil)
            if (image != nil) {
