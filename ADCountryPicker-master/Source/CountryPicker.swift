...
open class CountryPicker: UITableViewController {
    ...
    fileprivate var _sections: [CountryPickerSection]?
    fileprivate var sections: [CountryPickerSection] {
        ...
        var sections = [CountryPickerSection]()
        for _ in 0..<self.collation.sectionIndexTitles.count {
            sections.append(CountryPickerSection())
        }
        ...
        sections.insert(CountryPickerSection(), at: 0)
        let locale = Locale.current
        let displayName = (locale as NSLocale).displayName(forKey: NSLocale.Key.countryCode, value: countryCode)
        let countryData = CallingCodes.filter { $0["code"] == countryCode }
        let country: Country
    ...
    public func getFlag(countryCode: String) -> UIImage? {
        ...
        if countries.count > 0 {
            ...
            return UIImage(named: bundle + countries.first!.code.uppercased() + ".png",
                           in: Bundle(for: CountryPicker.self), compatibleWith: nil)
        }
        else {
            return nil
        }
